...
OpenWiki(https://github.com/langchain-ai/openwiki)와
Open Knowledge Format(https://github.com/GoogleCloudPlatform/open-knowledge-format)을
참고하고, 이 저장소의 README.md, SCHEMA.md, AGENTS.md를 읽어줘.

이 프로젝트에 관련된 모든 내용을 정리할 건데, 아래 문서 모음과 관련된
Claude Code·Codex 세션 전체를 분석하고, 사용하는 메모리 기록과 Git commit
이력도 source로 여기고 LLM wiki를 만들어줘.

- 문서 폴더: [문서 폴더 경로]
- 프로젝트/Git 저장소: [경로 또는 없음]
- 관련 세션: [Claude Code/Codex 및 프로젝트 경로, 또는 없음]
- 메모리 기록: [Claude mem 등 위치/접근 방법, 또는 없음]
- 제외할 자료: [제외 범위 또는 없음]
- Obsidian Vault: [경로, 없으면 새로 만들기, 모르면 함께 확인]

먼저 대상 프로젝트의 현재 레포 구조, 문서 배치, 기존 위키와 에이전트 규칙을
살펴봐줘. 내가 지정한 경로나 프로젝트의 기존 연결을 통해 Obsidian Vault와
위키 폴더·심링크·설정을 확인하고, 모르는 위치는 필요한 것만 물어봐줘.

위키는 기본적으로 docs/wiki 하위에 정리하되, 기존 레포와 Vault 구조에 맞는
정본 위치를 먼저 정해줘. Obsidian으로 해당 폴더를 열거나 기존 Vault에 연결해서
나와 에이전트가 같은 Markdown을 읽고 수정하는 협업 구조를 만들어줘.
위키 사본을 이중으로 관리하지 말고, 기존 문서·Vault 설정을 보존해줘.
원문은 원래 위치에 두고 정리된 지식에서 출처로 돌아갈 수 있게 해줘.
사람이 Obsidian에서 수정한 내용도 다음 작업 전에 다시 읽고 변경 근거에 반영해줘.
에이전트의 이전 초안으로 내 편집을 덮어쓰지 말고, 양쪽 편집이 겹치면 차이를
보여줘. 연결 후 같은 파일이 보이는지, 내부 링크·출처를 따라갈 수 있는지 확인해줘.

wiki.example.toml을 참고해 wiki.toml에 위키 위치·원천 범위·내 검토자 ID를
설정하고 scripts/setup.py로 작업 공간을 준비해줘. 예제 원천 대신 내가 지정한
실제 원천을 설정해줘. 다른 레포에 위키를 두면 그곳에 설정을 만들고
WIKI_CONFIG로 연결해줘.
문서·세션은 내 프로젝트에 해당하는 범위만 조사하고, 현재 검토 대화의
Claude/Codex 세션도 실제 사용자 판단을 기록할 수 있도록 연결해줘.

자료별 요약을 쌓는 데서 끝내지 말고 개념·대상·질문을 연결해줘.
사실과 해석, 계획과 실행 결과를 구분하고, 충돌과 미확인은 드러내줘.
처음 조사한 범위와 빠진 자료를 기록하고, skills/wiki-ingest와 wiki-review에
따라 변경안을 만든 뒤 구조·의미를 검토해줘. 내가 판단할 내용을 근거와 함께
한 번에 보여주고, 내 결정 후 승인한 버전만 반영해줘.

이후에는 이 저장소의 wiki-update·wiki-ingest·wiki-review·wiki-query 스킬로
갱신하고 질문할 수 있도록 연결해줘. 사용 가능한 세션이나 도구가 없으면
그 범위를 알려주고, 접근 가능한 자료의 준비는 계속해줘.
```
